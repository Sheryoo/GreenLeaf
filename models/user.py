...
def register_user(email, password, fullName, city, phoneNumber):
    '''Function to register user in the database'''
    conn = create_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO users (email,password,fullName,city, phoneNumber) VALUES (?,?,?,?,?)",
                   (email, password, fullName, city, phoneNumber))
    conn.commit()
    cursor.close()
    conn.close()
